...
async function checkAllPrerequisites(): Promise<boolean> {
  ...
  if (!allGood) {
    console.log(c.error("Some prerequisites are missing. Please fix them and run again."));
  } else {
...
async function createProject(projectName: string): Promise<void> {
  ...
  pkg.scripts.dev = "bunx convex dev --once && concurrently -r bun:dev:web bun:dev:convex";
  ...
  try {
    ...
    console.log(`  Run manually: gh repo create ${projectName} --private --source=. --remote=origin --push\n`);
  }
  ...
  try {
    ...
    console.log(c.warning("Could not initialize Convex automatically. Run: bunx convex dev\n"));
  }

  console.log("");
  console.log(c.warning("ACTION REQUIRED: Create a Convex Deploy Key"));
  console.log("\n  1. Go to: https://dashboard.convex.dev");
  console.log("  2. Select your project → Settings → Deploy Keys");
  console.log("  3. Create and copy the key\n");

  // Step 10: Link to Vercel
  ...
  try {
    ...
    console.log(c.warning("Could not link Vercel automatically. Run: bunx vercel link\n"));
  }

  console.log("");
  console.log(c.warning("ACTION REQUIRED: Add Environment Variables in Vercel"));
  console.log("\n  1. Go to: https://vercel.com/dashboard");
  console.log(`  2. Select: ${projectName} → Settings → Environment Variables`);
  console.log("  3. Add:");
...
`);
  ...
  if (!/^[a-zA-Z][a-zA-Z0-9_-]*$/.test(projectName)) {
    console.log(c.error("Invalid project name. Use letters, numbers, hyphens, underscores. Start with a letter."));
    process.exit(1);
  }
